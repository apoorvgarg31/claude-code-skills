...
**Orchestrator:** Opus (Claude Code) — always manages the workflow

**Available agents:**
- `codex` — OpenAI Codex CLI (fast coding)
- `claude` — Claude Code (thorough analysis)
- `gemini` — Gemini CLI (Google's AI)
- `aider` — Aider (git-integrated coding)
- `droid` — Droid CLI (autonomous coding)
- `opus` — Handle directly in this session
...
This will be used as the folder name under `.orchestra/`.
...
Write `./.orchestra/<project-name>/config.yaml`:
```yaml
...
Config saved to: ./.orchestra/config.yaml
